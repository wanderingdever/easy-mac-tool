...
可选启用的系统监控模块，1 秒轮询采集 7 项指标。默认关闭，需在设置中主动开启。

- **支持的指标**：
  - CPU 占用（基于 host_statistics 两次采样差值）
  - CPU 温度（IOKit SMC `TC0P`/`TC0D` key）
  - 风扇转速（IOKit SMC `F0Ac`/`FNum` key，多风扇）
  - 内存使用（vm_statistics64 active+wired+compressed）
  - 磁盘占用（URLResourceValues 启动盘）
  - 网速上下行（getifaddrs AF_LINK 接口采样差值）
  - GPU 占用（Metal 公开 API 不暴露 utilization，暂返回 nil 隐藏）
- **菜单栏展示**：勾选的指标在 E 图标旁用 `|` 分隔显示，常态单色跟随系统深浅色，CPU>80°C/80% 等异常值变橙红
- **下拉面板**：菜单栏下拉顶部展示监控面板，第一排 CPU 温度/内存/网速卡片，第二排磁盘占用进度条
- **机型兼容降级**：SMC 读取失败（虚拟机或部分机型）静默隐藏对应项不报错
- **零依赖**：直接 IOKit SMC 调用，无第三方框架，符合小而美原则

### 4. 菜单栏常驻

- `LSUIElement=YES`，无 Dock 图标
- 菜单栏显示 SF Pro Rounded 字体绘制的「E」图标（template 模式自适应深浅色）
- 监控启用时 E 图标旁可展示选中指标数值（混合样式：常态单色、异常着色，用 `|` 分隔）
- 下拉菜单底部「设置」「退出」按钮去掉文字、只保留图标，左右布局
