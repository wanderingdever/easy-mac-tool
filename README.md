...
- **实时跟随**：打开期间监听应用/窗口/Space 变化，自动刷新窗口列表并保持当前选中项
- **悬停预瞄 + 点击提交**：鼠标 hover 仅显示浅色边框预瞄，点击才真正选中并激活（与 Windows 一致）
- **悬停自动选中**：可开启，鼠标悬停直接移动键盘选中项；点击仍然立即激活
- **触控板滑动切换**：可开启三指左右滑动呼出切换器；已打开时继续滑动切换选中项
- **窗口控制**：聚焦/悬停缩略图时，在标题行右侧显示 macOS 红绿灯式关闭 / 最小化 / 全屏按钮
- **展示模式**：缩略图 / 应用图标 / 标题三种样式可切换
- **释放行为**：
  - `释放后聚焦`：松开修饰键即激活当前选中窗口（默认）
  - `按住（Enter 确认）`：保持面板，按 Enter 才激活
- **多屏支持**：面板出现位置可选「活跃屏幕 / 鼠标所在屏 / 主屏幕」
- **精确 Space 过滤**：通过运行时 SkyLight 桥接读取每窗口 Space 成员；私有 API 不可用时回退 on-screen 近似
- **过滤与排序**：支持全部/活跃/非活跃应用、最近聚焦/字母顺序、全屏与无窗口应用过滤
- **主窗口模式**：可选每个应用仅显示一个主窗口
- **标签页模式**：可选合并同应用同标题窗口，或显示为独立窗口
- **应用占位**：最小化/隐藏/无窗口的应用优先显示最后已知预览，无缓存时显示应用图标
...
- 剪贴板捕获关闭时停止轮询定时器；开启时按 0.5s / 1.5s 自适应频率轮询
- 切换器打开期间的实时刷新只补拍缺失预览，不重复抓取已有缩略图
- 图片与图标缓存均设字节/数量上限，系统内存压力或热状态变化时自动收紧
- 系统监控与窗口布局默认关闭，关闭状态不采样、不安装全局事件监听
- 网络链接预览有 6s 超时、512KB HTML / 256KB 图标截断，不缓存到磁盘
...
- 最小化/隐藏窗口无法实时捕获，只显示最后已知预览；无缓存时显示应用图标
